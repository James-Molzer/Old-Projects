...
select * from site
select to_date - from_date as Total_Date from reservation 

select DATEDIFF(DAY, from_date, to_date ) * daily_fee as tolal_cost from reservation join site on reservation.site_id = site.site_id join campground on campground.campground_id = site.campground_id 
where reservation_id = ( select MAX(reservation_id) from reservation) 
--select SCOPE_IDENTITY();
select * from reservation where reservation_id = '44'
select * from site where site_id = '46'
select * from campground where campground_id= '6'
select DATEDIFF(DAY, '2003-5-5', '2003-5-15' ) * daily_fee as tolal_cost from campground where campground.campground_id= 1
